...
async fn generate_creates_active_version_1() {
    ...
    let version = kp.generate_dek(controller, "tenant_brand").await.unwrap();
    assert_eq!(version, 1);
...
#[tokio::test]
async fn active_dek_is_stable_across_calls() {
    ...
    let controller = ControllerId::Tenant(Uuid::nil());
    kp.generate_dek(controller, "tenant_brand").await.unwrap();
    let a = kp.active_dek_for(controller, "tenant_brand").await.unwrap();
    let b = kp.active_dek_for(controller, "tenant_brand").await.unwrap();
    assert_eq!(a.0.bytes(), b.0.bytes());
    assert_eq!(a.1, b.1);
}

#[tokio::test]
async fn fresh_providers_produce_different_deks() {
    ...
    kp_b.generate_dek(controller, "tenant_brand").await.unwrap();
    let dek_a = kp_a
...
#[tokio::test]
async fn different_controllers_get_different_deks() {
    let kp = InMemoryKeyProvider::from_master_bytes(master());
    let a = ControllerId::Tenant(Uuid::from_u128(1));
    let b = ControllerId::Tenant(Uuid::from_u128(2));
    kp.generate_dek(a, "tenant_brand").await.unwrap();
    kp.generate_dek(b, "tenant_brand").await.unwrap();
    let dek_a = kp.active_dek_for(a, "tenant_brand").await.unwrap().0;
    let dek_b = kp.active_dek_for(b, "tenant_brand").await.unwrap().0;
    assert_ne!(dek_a.bytes(), dek_b.bytes());
}

#[tokio::test]
async fn tenant_and_user_with_same_uuid_get_different_deks() {
    ...
    kp.generate_dek(as_tenant, "default").await.unwrap();
    kp.generate_dek(as_user, "default").await.unwrap();
    let dek_tenant = kp.active_dek_for(as_tenant, "default").await.unwrap().0;
    let dek_user = kp.active_dek_for(as_user, "default").await.unwrap().0;
    assert_ne!(dek_tenant.bytes(), dek_user.bytes());
}

#[tokio::test]
async fn different_record_kinds_get_different_deks() {
    ...
    let dek_default = kp.active_dek_for(controller, "default").await.unwrap().0;
    let dek_safety = kp.active_dek_for(controller, "safety").await.unwrap().0;
...
#[tokio::test]
async fn rotate_chain_extends_version_space() {
    ...
    for expected_version in 2..=5u32 {
        let (new_v, _) = kp.rotate_dek(controller, "tenant_brand").await.unwrap();
...
#[tokio::test]
async fn dek_at_unknown_version_fails() {
    let kp = InMemoryKeyProvider::from_master_bytes(master());
    let controller = ControllerId::Tenant(Uuid::nil());
    kp.generate_dek(controller, "tenant_brand").await.unwrap();
    let err = kp
...
#[tokio::test]
async fn shred_unknown_version_fails() {
    let kp = InMemoryKeyProvider::from_master_bytes(master());
    let controller = ControllerId::Tenant(Uuid::nil());
    kp.generate_dek(controller, "tenant_brand").await.unwrap();
    let err = kp
...
#[tokio::test]
async fn different_master_keys_yield_independent_deks() {
...
#[tokio::test]
async fn master_file_loads_exact_32_bytes() {
    ...
    // pattern.
    let (_, version) = kp.active_dek_for(controller, "tenant_brand").await.unwrap();
    assert_eq!(version, 1);
}

#[tokio::test]
async fn master_file_rejects_short_file() {
    let dir = tempdir_for_test();
...
#[tokio::test]
async fn master_file_rejects_long_file() {
    let dir = tempdir_for_test();
...
#[tokio::test]
async fn master_file_rejects_missing_path() {
