...
| [ADR-014](ADR-014-frontend-architecture.md) | Frontend architecture — two SvelteKit codebases (tenant + staff) + one Flutter; shared `apps/web-ui` primitives; design tokens as web↔mobile bridge; embedded-static handshake; wrapped generated clients | Superseded by [ADR-020](ADR-020-mash-frontend-architecture.md) | 2026-06-02 |
...
| [ADR-019](ADR-019-white-label-runtime.md) | White-label runtime — saved tenant brand served as immutable content-hashed CSS asset via `<link>` to Host-keyed `/api/v1/tenant/brand.css` (302 to immutable URL); editor preview is JS CSSOM mutation on nonce-CSP sensitive route; scope bounded to brand primary/accent/surface-tint with aviation safety carve-outs; refines [ADR-014 §F](ADR-014-frontend-architecture.md) brand injection mechanism; composes with [ADR-015 §B/§C/§E](ADR-015-csp-static-build.md) without widening CSP | Accepted | 2026-06-08 |
| [ADR-020](ADR-020-mash-frontend-architecture.md) | Frontend architecture — Maud + HTMX + Alpine + Tailwind on Axum, CloudFront edge with ACM wildcard cert + WAF; single Rust binary serves JSON API and HTML surface in parallel; static/dynamic split (prerendered marketing, SSR per-user, SSE real-time); aviation safety carve-outs (MOR/AOG/ADD/dispatch/magic-link) MUST work no-JS and are CI-enforced; supersedes [ADR-014](ADR-014-frontend-architecture.md); refines [ADR-001 §F](ADR-001-platform.md), [ADR-002 §H](ADR-002-release-deployment.md), [ADR-005](ADR-005-workspace-layout.md), [ADR-015 §B/§C/§E](ADR-015-csp-static-build.md), [ADR-019 §E](ADR-019-white-label-runtime.md) | Accepted | 2026-06-08 |
| [ADR-021](ADR-021-cdn-front-door.md) | CDN front-door + origin runtime contract — Cloudflare provides DNS + CDN + WAF + L3/L4 DDoS at the edge; origin substrate is defined by interfaces (k3s, Cilium CNI + host networking, Istio ambient, CNPG, S3-compatible object store, cert-manager) not by vendor; ADR-001 §A's vendor-specific origin references read as instances of the runtime contract; specific provider selection lives in infra-repo runbooks; refines [ADR-001 §A](ADR-001-platform.md), [ADR-019 §B](ADR-019-white-label-runtime.md), [ADR-020 §H](ADR-020-mash-frontend-architecture.md) | Accepted | 2026-06-11 |
| [ADR-022](ADR-022-pluggable-aead.md) | Pluggable AEAD selection — three AEADs ship in `flight-academy-store` (AES-256-GCM-SIV default, ChaCha20-Poly1305, AES-256-GCM) behind a common `AeadCipher` trait; per-ciphertext algorithm-tag byte enables forward migration without schema change; AAD binds ciphertext to `record_kind:record_id:column`; CSPRNG nonces; refines [ADR-001 §D](ADR-001-platform.md) AEAD choice | Accepted | 2026-06-18 |
| [ADR-023](ADR-023-dek-lifecycle-rotation.md) | DEK lifecycle, versioning, and rotation — wrapped DEK per `(controller, record_kind, version)` in `tenant_dek_wrappings` / `user_dek_wrappings` tables; envelope format bump to carry `dek_version`; rotation as INSERT-new + UPDATE-retire + chunked sweep + DELETE shred; KEK rotation is row-rewrap with data untouched; eager DEK generation at controller create; refines [ADR-001 §D](ADR-001-platform.md) storage shape and [ADR-022 §B](ADR-022-pluggable-aead.md) envelope format | Accepted | 2026-06-19 |
